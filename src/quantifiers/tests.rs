use super::*;
use crate::instantiation::engine::generate_quantified_candidates;
...
fn provenance_preserves_nested_shadowing_and_lowered_bindings() {
    ...
    let (scoped, mut provenance) =
        crate::quantifiers::provenance::scope_model(input.clone(), true).unwrap();
    let (without_profile, empty) =
        crate::quantifiers::provenance::scope_model(input, false).unwrap();
...
fn provenance_tracks_property_witnesses_without_binder_rules() {
    ...
    let (scoped, mut provenance) =
        crate::quantifiers::provenance::scope_model(input, true).unwrap();
...
impl crate::cost_functions::YardbirdCostFunction<TermLanguage> for PreferLongName {
...
impl crate::cost_functions::YardbirdCostFunction<TermLanguage> for CountCosts {
    fn get_string_terms(&self) -> Vec<String> {
        vec![]
    }
    fn get_reads_and_writes(&self) -> smt2parser::vmt::ReadsAndWrites {
...
pub(super) fn prepared_options() -> InstantiationOptions {
    InstantiationOptions {
        ...
        candidate_scope: crate::instantiation::scope::CandidateScope::AllCandidates,
        refinement_step: 0,
        selection_counts: Default::default(),
        depth: 0,
        instrumentation: crate::instantiation::engine::InstantiationInstrumentation {
...
fn binder_continuation_finds_a_violation_after_a_satisfied_prefix() {
    use crate::instantiation::{
        grounding::instantiate_with_bindings,
        language::expr_to_term,
        search::{search_binder_page, BinderSearchCursor},
    };
...
fn binder_work_limit_is_reported_as_budget_exhaustion_not_continuation() {
    use crate::instantiation::search::{search_binder_page, BinderSearchCursor};
...
fn ranked_binder_fixture(phase: SearchPhase) -> InstantiationBatch {
    use crate::instantiation::{engine::InstantiationInstrumentation, scope::CandidateScope};
    let rule = BinderRule {
        name: "q".into(),
        kind: BinderKind::Forall,
        captures: vec![(Symbol("unit".into()), string_to_sort("Bool"))],
    ...
    generate_quantified_candidates(
        ...
        InstantiationOptions {
            ...
            instrumentation: InstantiationInstrumentation {
                artifact_capture: crate::instantiation::instantiator::ArtifactCapture {
                    decisions: true,
...
fn input_binders_use_costed_typed_egg_grounding_and_record_decisions() {
    ...
    let rendered = batch
        ...
        .map(|candidate| {
            ...
            crate::instantiation::language::expr_to_term(candidate.expression.clone()).to_string()
        })
        .collect::<Vec<_>>();
    assert!(rendered
        .iter()
...
fn input_binders_match_existing_body_atoms_before_domain_fallback() {
    ...
    let term =
        crate::instantiation::language::expr_to_term(triggered.candidates[0].expression.clone());
...
fn input_binder_batches_obey_rankers_budgets_and_novelty() {
    use crate::instantiation::{
        ranker::{InstantiationRanker, TermCostInstantiationRanker},
    ...
    impl InstantiationRanker for Reverse {
        ...
        fn compare(
            &self,
            left: &crate::instantiation::candidate::InstantiationCandidate,
            right: &crate::instantiation::candidate::InstantiationCandidate,
        ) -> std::cmp::Ordering {
