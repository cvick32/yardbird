...
use crate::policy::term_selection::{
...
use crate::rule_matching::rule::QuantifiedRuleCategory;
use crate::rule_matching::scope::CandidateScope;
use crate::terms::language::{translate_term, TermExpr, TermLanguage};
use crate::training::{
    canonical_term_hash, AbstractInstantiationRecord, CandidateRecord, DecisionRecord, TermFeatures,
};
...
{
    pub(crate) array_terms: crate::theories::array::term_index::ArrayTermIndex,
    ...
    CF: YardbirdCostFunction<TermLanguage>,
{
        ...
        N: egg::Analysis<TermLanguage>,
    {
        ...
        let mut array_terms = crate::theories::array::term_index::ArrayTermIndex::default();
        ...
        for raw_term in &candidate_catalog.source_grounded.terms {
            ...
            array_terms.admit_source_term(&term);
        ...
        array_terms.index_writes(
            &candidate_catalog.source_grounded.reads_and_writes,
    ...
    pub(crate) fn source_term_candidates(&self, id: egg::Id) -> Option<&Vec<RankedTerm>> {
        self.source_term_map.get(&id)
    }
    ...
    pub fn abstract_instantiation_record(
        ...
        let substitution = substitution
            .iter()
            .map(
                |(variable, term)| crate::rule_matching::provenance::InstantiationSubstitution {
...
mod tests {
    ...
    use super::{
        ...
        CandidateOrigin, TermExtractor, TermExtractorOptions,
    };
    use crate::policy::term_selection::{
        CandidateSelectionContext, CandidateView, ContextualCandidateSelector, YardbirdCostFunction,
    };
