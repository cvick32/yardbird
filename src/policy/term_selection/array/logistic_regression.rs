...
use crate::policy::term_selection::{
    CandidateSelectionContext, CandidateView, ContextualCandidateSelector, TermCostFactory,
    YardbirdCostFunction,
};
use crate::terms::language::TermLanguage;
use crate::training::{
    canonical_term_hash, LogisticRegressionCandidateFeatures, LogisticRegressionModel, TermFeatures,
};
...
impl TermCostFactory for LogisticRegression {
    type Config = LogisticRegressionModel;

    fn from_context(smt: &TermCostContext, depth: u32, model: &Self::Config) -> Self {
        Self::new(
            depth,
