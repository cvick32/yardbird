    ...
    for benchmark in data.get("benchmarks", []):
        ...
        if benchmark["result"]:
            ...
            benchmark_groups[base_name][strategy] = {
                ...
                "depth": result["depth"],
                "result_type": list(result["result"].keys())[0] if result["result"] else "unknown"
            }
...
def generate_tikz_scatter(points: List[TikzPoint], title: str = "Runtime Comparison") -> str:
...
"""
    ...
    for point in sorted_points:
        ...
        table_code += f"{point.label} & {point.x:.3f} & {point.y:.3f} & {speedup_str} \\\\\n"
...
def main():
    parser = argparse.ArgumentParser(description="Generate TikZ code from benchmark JSON")
    parser.add_argument("json_file", help="Benchmark results JSON file")
    parser.add_argument("--output-dir", help="Output directory for TikZ files", default=".")
